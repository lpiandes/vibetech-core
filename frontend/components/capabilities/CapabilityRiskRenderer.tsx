import { useContext } from "react";
import { CapabilityViewModelContext } from "./CapabilityContext";

export default function CapabilityRiskRenderer() {
  const vm = useContext(CapabilityViewModelContext);
  const risks = (vm?.risks ?? []) as any[];

  if (!risks.length) {
    return (
      <section className="rounded-xl border border-border bg-card p-4">
        <div className="text-sm font-semibold">Capability risks</div>
        <div className="mt-2 text-sm text-muted-foreground">No capability risks require attention.</div>
      </section>
    );
  }

  return (
    <section className="rounded-xl border border-border bg-card p-4">
      <div className="text-sm font-semibold">Capability risks</div>
      <div className="mt-3 space-y-2">
        {risks.map((r) => (
          <div key={r.id} className="rounded-lg border border-border/60 bg-background p-3">
            <div className="text-sm font-medium">{r.type}</div>
            <div className="mt-1 text-xs text-muted-foreground">{r.message}</div>
          </div>
        ))}
      </div>
    </section>
  );
}

