...
export async function GET(_req: Request, { params }: { params: Promise<{ businessId: string }> }) {
  try {
    ...
    const open = accessRequestService.store.listOpen(businessId);
    const mine = open.filter((entry: any) => String(entry.requesterUserId) === String(ctx.user.id));
    const forOwner = ctx.role === "OWNER" || ctx.authz.isPlatformAdmin ? open : mine;
...
export async function POST(request: Request, { params }: { params: Promise<{ businessId: string }> }) {
  try {
    ...
    const result = accessRequestService.requestAccess({
      businessId,
      requesterUserId: ctx.user.id,
      requestKind: String(body.requestKind ?? "module"),
      reason,
    ...
    if (!result.ok) {
      return NextResponse.json({
        ...
        productError: presentProductError("An identical access request is already open."),
