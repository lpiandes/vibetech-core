...
import { resolvePostInviteRedirect } from "@/lib/builder/resolvePostInviteRedirect";
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (validation.reason === "accepted" && validation.invitation) {
      ...
      if (sessionUser) {
        ...
        if (membership?.status === "ACTIVE") {
          const routing = await resolvePostInviteRedirect({
            platformStore,
            getAiBuilderService,
    ...
    const routing = await resolvePostInviteRedirect({
      ...
      getAiBuilderService,
      businessId,
      membershipRole: result.membership?.role ?? validation.invitation!.role,
      actorUserId: user.id,
      businessName: business?.name ?? null,
    });
    const redirectTo = routing.redirectTo;
    ...
    return NextResponse.json({
      ...
      userId: user.id,
      sessionEstablished,
      architectSessionId: routing.architectSessionId ?? null,
    });
  } catch (err) {
