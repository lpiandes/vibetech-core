...
export async function requireFeRetentionContext(businessId: string) {
  const scope = await getAuthorizedBusinessScope(businessId);
  ...
  if (!businessGrantsFeRetentionAccess(packages) && !scope.isPlatformAdmin) {
...
export function feJsonError(err: unknown) {
  if (err instanceof AuthorizationError) {
    const status = err.code === "UNAUTHENTICATED" ? 401 : err.code === "NOT_FOUND" ? 404 : 403;
