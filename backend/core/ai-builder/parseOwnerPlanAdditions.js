/**
 * Parse free-text owner add requests into plan additions that show immediately.
 * Shared by API + Architect UI.
 */
...
export function parseOwnerPlanAdditions(text = "") {
  ...
  // Full-sentence capability request → one teammate (what owners usually mean).
...
function looksLikeCapabilitySentence(text) {
...
function summarizeEmployeeLabel(text) {
...
function looksLikeEmployee(text) {
...
export function mergePlanAdditions(existing = {}, next = {}) {
  const modules = [
