...
export function buildDigitalEmployeeReadiness({
  ...
  const requiredConnections = safeArray(def.connectionDependencies?.length
    ? def.connectionDependencies
    : connectedSystemsSnapshot?.connections?.filter((c) => c.requirementLevel === "required").map((c) => c.id));
  ...
  if (missingCapabilities.length === 0 && missingKnowledge.length === 0 && missingConnections.length === 0 && missingIntegrationCapabilities.length === 0 && !def.requiresApproval) {
